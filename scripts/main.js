...
function updateBookmark(hikeID) {
    ...
    currentUser.get().then(
        (userDoc) => {
            ...
            let isBookmarked = bookmarks.includes(hikeID); //check hikeID in bookmarks array
            ...
            if (isBookmarked) {
                currentUser.update({
                    ...
                    console.log(`item was removed from database: ${hikeID}`);
                    document.getElementById(iconID).innerText = 'bookmark_border';
                })
                console.log(`item is exist: ${isBookmarked}`);
            } else {
                console.log(`item is exist: ${isBookmarked}`);
                currentUser.update({
                    bookmarks: firebase.firestore.FieldValue.arrayUnion(hikeID)
                }).then(() => {
                    console.log(`item was add to database: ${hikeID}`);
                    document.getElementById(iconID).innerText = 'bookmark';
                })
            }
        })
...
//     // console.log("in save saveBookMarker ");
//     // handle the backend: save the hikeID in the firestore
//     currentUser.update(
//         {bookmarks: firebase.firestore.FieldValue.arrayUnion(hideID)}
//     ).then(
//         ()=>{
//             console.log("save"+hikeID);
//             let iconID = 'save-'+hikeID;
//             document.getElementById(iconID).innerText = 'bookmark';
//         }
//     )
